...
class wechatCallback
{
    ...
    public function responseMsg()
    {
		...
		if (!empty($postStr)){
			 ...
			 * 如学号绑定成功后(有加连深大云主页面的这种需要紧急响应的情况，却不需要用户
            ...
            if ($recMsgType == "text"){							//接收到用户的文本消息
				...
				if(!empty( $keyword ) || $keyword == "0")			
                {
					...
					if ($action == "wechat_main"){
						...
						if ($keyword == "深大云" || $keyword == "1"){
							$code = "0";														//启动activity_main
						}else if ($keyword == "励志创业"  || $keyword == "励志" || $keyword == "2"){
							$code = "160";
						}/*else if ($keyword == "校园资询" || $keyword == "校园" || $keyword == "3"){
							$code = "130";
						}else if ($keyword == "新生手册"  || $keyword == "新生" || $keyword == "4"){
							$code = "140";
						}else if ($keyword == "数码天地"  || $keyword == "数码" || $keyword == "5"){
							$code = "150";
						}else if ($keyword == "云商城"  || $keyword == "商城" || $keyword == "6"){
							$code = "170";
						}*/else if ($keyword == "查课表"  || $keyword == "查课程" || $keyword == "3"){
							//$code = "30";
							$code = "20";
						...
						/*else if ($keyword == "微社区" || $keyword == "深大云微社区" || $keyword == "6"){
							$code = "64";
						}*/
						else if ($keyword == "天气资讯"  || $keyword == "天气" || $keyword == "4"){
							$code = "70";
						}/*else if ($keyword == "快递查询"  || $keyword == "快递" || $keyword == "9"){
							$code = "80";
						}*//*else if ($keyword == "周边生活"  || $keyword == "周边" || $keyword == "9"){
							$code = "90";
						}*//*else if ($keyword == "绑定学号" || $keyword == "10"){
							$code = "20";
					 ...
					 *-显示，并没有用户交互选项，所以可以在交互页面显示这里附加上深大云或者其他
					 *-交互页面。
					 *///////////////////////////////////////////////////////////////////////
					/*
					 * 深大云主页
					 ...
					 * 只是借用，还是跟主界面交互，所以这里有必要实时强制为监听深大云主页面消息，故而上面的事件交互码判断就可以不用加绑定学号  
					 * 成功这项了
					 */	
						/*
						//文本查课表
						...
						$content = WINDOWS::$type['bindToStudentSuccess'].$content;*/

						/*
						 * 图文查课表
						 */
						$studentNo = getStudentNoWithOpenId($fromUsername);
						$url = "%s/mobile/index.php?action=%s&student_no=%s";
						$url = sprintf($url, URL::$LOCAL, "show_lesson", $studentNo);					//传递GET类型
						$studentName = getNameWithStudentNo($studentNo);

						$sender_type = "picText";			//图文方式发送
						$temp = "
问：下节课是什么？在哪里上？
答：关注[深大云]，即刻拥有掌上云课表。";		//描述
						$data = mb_convert_encoding("点击查看", "UTF-8", "gbk").$studentName.mb_convert_encoding("的课表", "UTF-8", "gbk");
						$picText_list[0]['Title'] = $data;
						$picText_list[0]['PicUrl'] = URL::$LOCAL."/wechat/pic/lesson_show4.jpg";
						$picText_list[0]['Description'] = mb_convert_encoding($temp, "UTF-8", "gbk");
						$picText_list[0]['Url'] = $url;

						$code = "0";						//特殊:显示学号绑定成功后(包含主页面交互)，事件监听也应该同时处于深大云主页面状态
						$wechat_user_db->set_interact_code($fromUsername,$code);
					 ...
					 * 校园资询
					 */
					else if ($action == "school_news"){
						$content = WINDOWS::$type['school_news'];
						$code = "0";
						$wechat_user_db->set_interact_code($fromUsername,$code);//显示当前活动主页面，并监听
					}
					/*
					 * 新生手册
					 */
					else if ($action == "new_manual"){
						$content = WINDOWS::$type['new_manual'];
						$code = "0";
						$wechat_user_db->set_interact_code($fromUsername,$code);//显示当前活动主页面，并监听
					}
					/*
					 * 数码天地
					 */
					else if ($action == "digital_earth"){
						/*
						 * 文本
						 */
						$content = WINDOWS::$type['digital_earth'];
						/*
						 * 图文
						 */
						/*
						$sender_type = "picText";			//图文方式发送   
						$data = mb_convert_encoding("查课表", "UTF-8", "gbk");
						$picText_list[0]['Title'] = $data;
						$picText_list[0]['PicUrl'] = URL::$LOCAL."/wechat/pic/lesson_show.jpg";
					 ...
					 * 励志创业
					 */
					else if ($action == "encourage_work"){
						$content = WINDOWS::$type['encourage_work'];
						$code = "0";
						$wechat_user_db->set_interact_code($fromUsername,$code);//显示当前活动主页面，并监听
					}
					/*
					 * 云商城
					 */
					else if ($action == "cloud_shop"){
						$content = WINDOWS::$type['cloud_shop'];
						$code = "0";
						$wechat_user_db->set_interact_code($fromUsername,$code);//显示当前活动主页面，并监听
					}
					/*
